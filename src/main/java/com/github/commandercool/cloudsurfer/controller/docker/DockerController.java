...
public class DockerController {
    ...
    @RequestMapping(path = "/download/aseg2table/xls", method = RequestMethod.GET)
    public @ResponseBody ResponseEntity downloadAseg2TableXls(@RequestParam(name = "tag") String tag) throws Exception {
        final String id = dockerService.runAseg(subjectStorageService.fetchSubjectsByTag(tag), tag);
        while (dockerService.isRunning(id)) {
//             TODO: limit here
        }
        return ResponseEntity.ok().contentType(MediaType.parseMediaType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"))
                .header("Content-Disposition", "attachment; filename=" + tag + ".xlsx")
                .body(new InputStreamResource(mriService.downloadAsegTableXls(tag)));
    }
